...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
//    int *test = 0;
//    i2c_read_bytes(104, 27, 1, &test, 0);
//    UARTprintf("G_FSR: %04x\r\n",&test[0]);
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)     // if DMP is NOT on
        return -1;
    else {                      // if DMP is on
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
//    i2c_read(st.hw->addr, st.reg->mem_r_w,length, test);
//    int i;
//    for(i=0;i<16;i++){
//        UARTprintf("Data[%d]: %d\r\n",i,test[i]);
//    }
        ...
        unsigned char *data)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
//    int *test = 0;
//    i2c_read(st.hw->addr, st.reg->bank_sel, 2, &test);
//    UARTprintf("T1_R : %d\r\n",&test[0]);
//    UARTprintf("T2_R : %d\r\n",&test[1]);
...
#define LOAD_CHUNK  (16)
    unsigned char cur[LOAD_CHUNK] = {0};
    unsigned char tmp[2];
    int i = 0;
//    for(i=0;i<16;i++){
//        UARTprintf("C_I[%d]: %d\r\n",i,cur[i]);
...
//        UARTprintf("Length: %d\r\n",(unsigned char*)&firmware[ii]);
        ...
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
//        if (i2c_read(st.hw->addr, st.reg->mem_r_w, this_write, &cur))
//                return -1;
//        UARTprintf("F[%d]: %d\r\n",i,firmware+ii);
//        UARTprintf("C[%d]: %d\r\n",i,cur);
//        UARTprintf("Count: %d\r\n",ii);
//        for(i=0;i<16;i++){
//            UARTprintf("F[%d]: 0x%02x\r\n",i,(firmware+ii)[i]);
//        }
//        for(i=0;i<16;i++){
//            UARTprintf("C[%d]: 0x%02x\r\n",i,cur[i]);
//        }
...
static int setup_compass(void)
{
    ...
    // added to use 0x0C if above code does not return a valid akm_addr
    akm_addr = 0x0C;
