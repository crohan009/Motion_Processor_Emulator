//#define ENABLE_MPU_INV_INTERFACE
...
static unsigned int mpl_rate = 1;
...
void mpu_get_outputs(MPU_MPL_outputs *mpu_mpl_outputs)
{
  ...
  double n = (1.0)*(1<<30);
  for (i=0; i<4; i++) mpu_mpl_outputs->quat[i] = data[i] * 1.0/n;
  mpu_mpl_outputs->quat_accuracy = accuracy;
  mpu_mpl_outputs->quat_timestamp = timestamp;

  inv_get_sensor_type_accel(data, &accuracy, (inv_time_t*)&timestamp);
  /* q16 fixed point to m/s^2. In fixed point representation, 1 g = 2^16
   * Hence, conversion constant is 9.80665 / 2^16 */
#define ACCEL_CONVERSION 0.000149637603759766f
  //    double n = (1.0)*(1<<16);
  for (i=0; i<3; i++) mpu_mpl_outputs->acc[i] = data[i] * ACCEL_CONVERSION;
  mpu_mpl_outputs->acc_accuracy = accuracy;
  mpu_mpl_outputs->acc_timestamp = timestamp;

  inv_get_sensor_type_gyro(data, &accuracy, (inv_time_t*)&timestamp);
  /* q16 fixed point to rad/sec. In fixed point representation, 1 dps = 2^16
   * Hence, conversion constant is pi / 2^16 / 180 */
#define GYRO_CONVERSION 2.66316109007924e-007f
  //    double n = (1.0)*(1<<16);
  for (i=0; i<3; i++) mpu_mpl_outputs->gyr[i] = data[i] * GYRO_CONVERSION;
  mpu_mpl_outputs->gyr_accuracy = accuracy;
  mpu_mpl_outputs->gyr_timestamp = timestamp;

  inv_get_sensor_type_compass(data, &accuracy, (inv_time_t*)&timestamp);
  /* q16 fixed point to uT. In fixed point representation, 1 uT = 2^16
   * Hence, conversion constant is 1 / 2^16 */
#define COMPASS_CONVERSION 1.52587890625e-005f
  //    double n = (1.0)*(1<<16);
  for (i=0; i<3; i++) mpu_mpl_outputs->mag[i] = data[i] * COMPASS_CONVERSION;
  mpu_mpl_outputs->mag_accuracy = accuracy;
  mpu_mpl_outputs->mag_timestamp = timestamp;
}
...
void mpu_get_euler(EulerAngles_outputs *angles)
{
    long data[3];
  int i;
  inv_get_sensor_type_euler(data, &angles->accuracy, (inv_time_t*)(&angles->timestamp));    // returns data[0] == pitch, data[1] == roll, data[2] == yaw
  double n = (1.0)*(1<<16);
#define DEG2RAD 0.0174532925199433
  for (i=0; i<3; i++) angles->angles[i] = data[i] * 1.0/n;// * DEG2RAD;
}
...
void mpu_get_rot_mat(RotationMatrix_outputs *R)
{
    long data[9];
  int i;
  inv_get_sensor_type_rot_mat(data, &R->accuracy, (inv_time_t*)(&R->timestamp));
  double n = (1.0)*(1<<30);
  for (i=0; i<9; i++) R->R[i] = data[i] * 1.0/n;
}
...
// disable tap and android_orient
...
{ 
//  UARTprintf("FIFo_Rate: %d\r\n",fifo_rate);
  inv_error_t result;
    unsigned char accel_fsr;                    // fsr --> Full Scale Range
  ...
  if (result) {
      UARTprintf("Could not initialize gyro.\n");
  }
  ...
  result = inv_init_mpl();
  if (result) {
      UARTprintf("Could not initialize MPL.\n");
  }
  ...
  if (result == INV_ERROR_NOT_AUTHORIZED) {
      while (1) {
          UARTprintf("Not authorized.\n");
      }
  }
  if (result) {
      UARTprintf("Could not start the MPL.\n");
  }
...
//    UARTprintf("MPU sample rate = %d\r\n",gyro_rate);
    mpu_get_gyro_fsr(&gyro_fsr);
//    UARTprintf("Gyro fsr = %d\r\n",gyro_fsr);
    mpu_get_accel_fsr(&accel_fsr);
//    UARTprintf("Acc fsr = %d\r\n",accel_fsr);

#ifdef COMPASS_ENABLED
    mpu_get_compass_fsr(&compass_fsr);
//    UARTprintf("Compass fsr = %d\r\n",compass_fsr);
#endif


//     mpu_set_lpf(42);         // Set MPU's Low Pass Filter to cut off high-freq noise >= 42 Hz
     mpu_set_lpf(98);         // Set MPU's Low Pass Filter to cut off high-freq noise >= 98 Hz
  ...
  /* Compass reads are handled by scheduler. */
     ...
     */
    int r = dmp_load_motion_driver_firmware();
//    UARTprintf("Firmware: %d\r\n",r);
    ...
    hal.dmp_features = DMP_FEATURE_6X_LP_QUAT
    		...
    		| DMP_FEATURE_TAP | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
        DMP_FEATURE_GYRO_CAL;
    dmp_enable_feature(hal.dmp_features);
    dmp_set_fifo_rate(fifo_rate);
    mpu_set_dmp_state(1);
    hal.dmp_on = 1;
    hal.lp_accel_mode = 0;
}
...
static unsigned long next_send_time;
...
int mpu_update(MPU_MPL_outputs *mpu_mpl_outputs)
{
    ...
    #ifdef ENABLE_MPU_INV_INTERFACE
        if (uart_byte_available() != 0)
        {
            unsigned char c = uart_get_byte();
            handle_input(c);
        }
    #endif
    ...
    while (hal.new_gyro) {
        // UARTprintf("i");
            unsigned char read_new_gyro_data = 0;

            // disable motion_int_mode
            #if 0
            ...
            if (hal.new_gyro && hal.lp_accel_mode) {
                 ...
                 */
                int e = dmp_read_fifo(gyro, accel_short, quat, &sensor_timestamp, &sensors, &more);

                if (e==0) read_new_gyro_data = 1;
                if (!more)
                    hal.new_gyro = 0;
                ...
                if (sensors & INV_WXYZ_QUAT) {
                    ...
                    /* char _s[128];
                    float n = 1 << 29;
                            sprintf(_s,"Quat: %.2f %.2f %.2f %.2f\n",quat[0]/n,quat[1]/n,quat[2]/n,quat[3]/n);
                    MPL_LOGI(_s); */
                }
            }
                 ...
                 */
                hal.new_gyro = 0;
                int e = mpu_read_fifo(gyro, accel_short, &sensor_timestamp,
                    &sensors, &more);
                if (e==0) read_new_gyro_data = 1;
                if (more){
                    hal.new_gyro = 1;
            ...
            if (new_data) {
                ...
                inv_execute_on_data();
                //long t2 = get_ms();
                mpu_get_outputs(mpu_mpl_outputs);
                //long t3 = get_ms();
                //char s[32];
                //sprintf(s,"T %ld %ld",t1,t2);
                //AP_send_msg(s);
                // UARTprintf("u");
                 data_updated = 1;
            }
            #ifdef ENABLE_MPU_INV_INTERFACE
                    if (timestamp > next_send_time) {
                         ...
                         */
                        read_from_mpl();
                        next_send_time = timestamp + 500; // 2 Hz update to computer
                    }
            #endif
    }
    return data_updated;
