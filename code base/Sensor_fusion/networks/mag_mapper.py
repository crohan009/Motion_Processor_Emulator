    ...
    def train_model(self, epochs, dsets_enqueuer_training, dsets_enqueuer_testing):
        ...
        for Epoch in range(epochs):
            ...
            for idx, data in enumerate(dsets_enqueuer_training, 1):
                ...
                if torch.cuda.is_available():
                    x, y = Variable(x.cuda(), requires_grad = True).float(), Variable(y.cuda(), requires_grad = False).float()
                else:
                    x, y = Variable(x, requires_grad = True).float(), Variable(y, requires_grad = False).float()
...
class Mag_mapper_Data_loader(Dataset):
    # Loads data into the Mag_mapper model
    ...
    def transform_data(self):
        trans = np.array([1, 1, 1, 
                          -1, -1, -1]).reshape((1,6)) # data specific transformation
    ...
    def __getitem__(self,idx ):        # idx ranges from 0 to len(self) 
        ...
        return {'x': self.data_matrix[i, :3], 'y': self.data_matrix[i, 3:]}

    

# if __name__ == "__main__":
    
#     model = Mag_mapper(init_weights=True)
#     data_enq_trial = Mag_mapper_Data_loader("../Datasets/MPU_rawtoypr_data_cleaned_test_data.csv", sequential_test_mode=True)
