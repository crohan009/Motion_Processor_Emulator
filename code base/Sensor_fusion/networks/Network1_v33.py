...
class Network1(nn.Module):
    ...
    def __init__(self, init_weights=False):
        ...
        self.name = "Net1_v3.3_lsm2mpu"

        # Inputs = 9; Outputs = 3; Hidden = (3 layers) 20, 30 ,10
        self.fc_y1 = nn.Linear(9, 8) 
        self.fc_y2 = nn.Linear(8, 7)   
        self.fc_y3 = nn.Linear(7, 6)
        self.fc_y4 = nn.Linear(6, 6)
        self.fc_y5 = nn.Linear(6, 5)
        self.fc_y6 = nn.Linear(5, 3)
        self.fc_y7 = nn.Linear(3, 1)

        self.fc_p1 = nn.Linear(9, 8) 
        self.fc_p2 = nn.Linear(8, 7)   
        self.fc_p3 = nn.Linear(7, 6)
        self.fc_p4 = nn.Linear(6, 6)
        self.fc_p5 = nn.Linear(6, 5)  
        self.fc_p6 = nn.Linear(5, 3) 
        self.fc_p7 = nn.Linear(3, 1)

        self.fc_r1 = nn.Linear(9, 8) 
        self.fc_r2 = nn.Linear(8, 7)   
        self.fc_r3 = nn.Linear(7, 6)
        self.fc_r4 = nn.Linear(6, 6)
        self.fc_r5 = nn.Linear(6, 5)  
        self.fc_r6 = nn.Linear(5, 3) 
        self.fc_r7 = nn.Linear(3, 1)
    ...
    def forward(self, x):
        yx = self.relu(self.fc_y1(x))
        px = self.relu(self.fc_p1(x))
        rx = self.relu(self.fc_r1(x))

        yx = self.relu(self.fc_y2(yx))
        px = self.relu(self.fc_p2(px))
        rx = self.relu(self.fc_r2(rx))

        yx = self.relu(self.fc_y3(yx))
        px = self.relu(self.fc_p3(px))
        rx = self.relu(self.fc_r3(rx))

        yx = self.relu(self.fc_y4(yx))
        px = self.relu(self.fc_p4(px))
        rx = self.relu(self.fc_r4(rx))

        yx = self.relu(self.fc_y5(yx))
        px = self.relu(self.fc_p5(px))
        rx = self.relu(self.fc_r5(rx))

        yx = self.relu(self.fc_y6(yx))
        px = self.relu(self.fc_p6(px))
        rx = self.relu(self.fc_r6(rx))

        yx = self.relu(self.fc_y7(yx))
        px = self.relu(self.fc_p7(px))
        rx = self.relu(self.fc_r7(rx))
    ...
    def train_model(self, epochs, dsets_enqueuer_training, dsets_enqueuer_testing):
        ...
        # Optimizer
        optimizer = optim.Adam(self.parameters(),lr = 0.0005, betas=(0.9, 0.999), eps=1e-08)
        ...
        for Epoch in range(epochs):
            ...
            for idx, data in enumerate(dsets_enqueuer_training, 1):
                ...
                if torch.cuda.is_available():
                    loss.backward(torch.Tensor([1, 1, 1]).cuda())
                else:
