...
int accel_lsm9_write_reg(uint8_t reg, uint8_t val)
{
    return i2c_write(ADDR_ACCEL_LSM9,reg,1,val);
}
#endif
...
#elif defined(HAS_LSM9)
    return i2c_write(ADDR_MAG_LSM9,reg,1,val);
#endif
}
...
#elif defined(HAS_LSM9)
    return i2c_write(ADDR_GYRO_LSM9,reg,1,val);
#endif
}
...
//	ret[5] = I2CReceive(ADDR_ACCEL_LSM9,0x2D);
    i2c_read_bytes(ADDR_ACCEL_LSM9,0x28,6,ret);
    return 1;
}
#endif
...
//	ret[5] = I2CReceive(ADDR_MAG_LSM9,0x0D);
     int a = i2c_read_bytes(ADDR_MAG_LSM9,0x08,6,ret);
#endif
	return 1;
}
...
//    ret[5] = I2CReceive(ADDR_GYRO_LSM9,0x2D);
     i2c_read_bytes(ADDR_GYRO_LSM9,0x28,6,ret);
#endif
    return 1;
}
