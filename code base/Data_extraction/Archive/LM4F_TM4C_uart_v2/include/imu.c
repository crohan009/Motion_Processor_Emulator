...
//	char x[100];
//	sprintf(x,"0x%2x\r\n",I2CReceive(ADDR_GYRO_LSM9,0x20));
//	send_msg(x);
	if (a < 0) return -1;
...
int accel_lsm9_write_reg(uint8_t reg, uint8_t val)
{
    return i2c_write_lsm(ADDR_ACCEL_LSM9,reg,1,&val);
}
#endif
...
#elif defined(HAS_LSM9)
    return i2c_write_lsm(ADDR_MAG_LSM9,reg,1,&val);
#endif
}
...
#elif defined(HAS_LSM9)
    return i2c_write_lsm(ADDR_GYRO_LSM9,reg,1,&val);
#endif
}
...
//    i2c_read_lsm(ADDR_ACCEL_LSM9,0x28,6,&ret);
    return 1;
}
#endif
...
//     int a = i2c_read_lsm(ADDR_MAG_LSM9,0x08,6,&ret);
#endif
	return 1;
}
...
//     i2c_read_lsm(ADDR_GYRO_LSM9,0x28,6,&ret);
//     char x[30];
//     sprintf(x,"%0.2f\r\n",ret[0]);
//     send_msg(x);
#endif
