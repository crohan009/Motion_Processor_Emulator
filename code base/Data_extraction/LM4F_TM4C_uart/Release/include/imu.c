...
//	scaledBuf->mag_data[AXIS_X] = (float)rawBuffer.mag_data[AXIS_X];                // here's the unscaled mag data
//    scaledBuf->mag_data[AXIS_Y] = (float)rawBuffer.mag_data[AXIS_Y];              // in case you need it
//    scaledBuf->mag_data[AXIS_Z] = (float)rawBuffer.mag_data[AXIS_Z];
...
int accel_lsm303_write_reg(uint8_t reg, uint8_t val)
{
    return i2c_write_reg(ADDR_ACCEL_LSM303,reg,val,&disable_303,f);
}
#endif
...
int accel_lsm330_write_reg(uint8_t reg, uint8_t val)
{
    return i2c_write_reg(ADDR_ACCEL_LSM330,reg,val,&disable_330,f);
}
#endif
...
int accel_lsm9_write_reg(uint8_t reg, uint8_t val)
{
    return i2c_write_reg(ADDR_ACCEL_LSM9,reg,val,f);
}
#endif
...
{
#ifdef HAS_LSM303
    return i2c_write_reg(ADDR_MAG_LSM303,reg,val,&disable_303,f);
#elif defined(HAS_LSM9)
    return i2c_write_reg(ADDR_MAG_LSM9,reg,val,f);
#endif
}
...
{
#ifdef HAS_LSM330
    return i2c_write_reg(ADDR_GYRO_LSM330,reg,val,f);
#elif defined(HAS_LSM9)
    return i2c_write_reg(ADDR_GYRO_LSM9,reg,val,f);
#endif
}
