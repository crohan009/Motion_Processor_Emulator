...
void initMPU9150()
{
 // wake up device
  ...
  // Configure Gyro and Accelerometer       (Page 10)
  // Disable FSYNC and set accelerometer and gyrometer bandwidth to 44 and 42 Hz, respectively;
  // DLPF_CFG = bits 2:0 = 010; this sets the sample rate at 1 kHz for both

//  i2c_write_reg(MPU9150_ADDRESS, CONFIG, 0x03,f);       // Minimum delay time is 4.9 ms which sets the fastest rate at ~200 Hz
  i2c_write_reg(MPU9150_ADDRESS, CONFIG, 0x00,f);       // this sets Minimum delay time to 0.98 ms which sets the fastest sampling rate to ~1000 Hz

  // Set sample rate = gyroscope output rate / (1 + SMPLRT_DIV)
//    i2c_write_reg(MPU9150_ADDRESS, SMPLRT_DIV, 0x04,f);           // sample rate = 1000 / (1 + 4) = 200 Hz
  i2c_write_reg(MPU9150_ADDRESS, SMPLRT_DIV, 0x07,f);               // sample rate = 8000 / (1 + 7) = 1000 Hz = 1 kHz
  ...
  i2c_write_reg(MPU9150_ADDRESS, GYRO_CONFIG, c | Gscale << 3,f);   // Set full scale range for the gyro ; currently set to +- 250 DPS
